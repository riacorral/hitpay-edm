...
function parseBody(body: string): EdmSection[] {
  ...
  while (i < lines.length) {
    ...
    // Skip standalone HTML open/close tags (e.g. <div style="...">, </div>)
    if (/^<\/?[a-zA-Z][^>]*>\s*$/.test(trimmed)) {
      i++;
      continue;
    }

    // HTML anchor → CTA: <a href="url" ...>Button Text</a>
    // AI sometimes generates styled HTML buttons instead of markdown CTAs
    const htmlAnchorMatch = trimmed.match(/^<a\s[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>\s*$/i);
    if (htmlAnchorMatch) {
      const ctaUrl = htmlAnchorMatch[1];
      const ctaText = htmlAnchorMatch[2].replace(/<[^>]+>/g, '').trim();
      if (ctaText && ctaUrl.startsWith('http')) {
        sections.push({ type: 'cta', text: ctaText, url: ctaUrl });
      }
      i++;
      continue;
    }
    ...
    if (paraLines.length > 0) {
      // Strip any HTML tags that leaked through (AI sometimes generates styled HTML)
      const text = paraLines.join(' ').replace(/<[^>]+>/g, '').trim();
